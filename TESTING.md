...
*   **Login Authentication:** Verified correct handling of valid credentials, error handling for invalid logins, JWT token generation, password hashing using bcrypt, and role-based access control (differentiating standard users from admins).
*   **API Routes:** Tested individual endpoint responses. For instance, testing user registration to ensure it successfully creates a user and handles duplicate email constraints.
*   **AI Assistant:** Tested the integration with the Google Gemini API, ensuring the system can process user inputs, send them to the API with correct system instructions (English/Malayalam), and gracefully handle rate-limit or invalid API key errors.
*   **Emission Calculation Logic:** Validated the mathematical formulas mapping activity inputs (e.g., fuel type, distance, food type, quantity) to CO₂ emissions using standard conversion factors.
...
*   **Usability Feedback:** Gathered feedback on the UI/UX design, such as the clarity of the emission charts, the ease of manually entering food/transport data, and the intuitiveness of the Chatbot interface.
*   **Language Support:** Tested the application with native Malayalam speakers to ensure the localized text is accurate and renders correctly across all screens.
*   **Mobile App Demonstration:** Followed the mobile build instructions to run the wrapped app on actual Android devices, collecting feedback on touch responsiveness and native-feel.
